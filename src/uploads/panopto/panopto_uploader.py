...
MANIFEST_FILE_TEMPLATE = settings.PANOPTO_UPLOAD_MANIFEST_TEMPLATES + 'upload_manifest_template.xml'

# Filename of manifest XML file. Any filename is acceptable.
MANIFEST_FILE_NAME = settings.PANOPTO_UPLOAD_MANIFEST_TEMPLATES + 'upload_manifest_generated.xml'
...
class PanoptoUploader:
    ...
    def __create_manifest_for_video(self, file_path, manifest_file_name, upload_title=None, upload_description=''):
        ...
        # Escape ampersands and replace angle brackets
        title = title.replace('&', '&#38;')
        upload_description = upload_description.replace('&', '&#38;')
        # Replace angle brackets with square brackets so that they don't
        # get treated as HTML tags by Panopto
        title = title.replace('<', '[').replace('>', ']')
        upload_description = upload_description.replace('<', '[').replace('>', ']')
    ...
    def __monitor_progress(self, upload_id):
        ...
        while True:
            ...
            if session_upload['State'] == 4: # Complete
                print(session_upload['SessionId'])
                if session_upload['SessionId'] is not None:
